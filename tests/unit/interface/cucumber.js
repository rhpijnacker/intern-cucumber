  ...
  return {
    before: function() {
      return mockRequire(require, 'src/interface/cucumber', {
        '@theintern/common': { Task, global: mockGlobal }
    ...
    tests: {
      ...
      'registerCucumber should add a suite'() {
        ...
        assert.equal(
          ...
          1,
          'addSuite should have been called once'
        );
        assert.lengthOf(
          rootSuite.tests,
          1,
          'There should be exactly one child suite'
        );
        assert.instanceOf(
          rootSuite.tests[0],
          Suite.default,
          'Child suite should be a suite instance'
        );
        assert.equal(
          rootSuite.tests[0].name,
          'dummy',
          'Child suite should have the right name'
        );
        assert.lengthOf(
          rootSuite.tests[0].tests,
          0,
          'Child suite should have no tests'
        );
      },
      ...
      'one scenario should give one sub-suite with one test case'() {
        ...
        return rootSuite.run().then(() => {
          ...
          assert.instanceOf(
            suite,
            Suite.default,
            'Sub-suite should be a suite instance'
          );
          ...
          assert.instanceOf(
            test,
            Test.default,
            'Test should be a test instance'
          );
          assert.equal(
            test.name,
            'A scenario',
      ...
      'a scenario outline should give multiple test cases'() {
        ...
        return rootSuite.run().then(() => {
          ...
          assert.equal(
            suite.tests[0].name,
            'A scenario with examples',
            'Test 1 should have the right name'
          );
          assert.equal(
            suite.tests[1].name,
            'A scenario with examples (2)',
            'Test 2 should have the right name'
          );
          assert.equal(
            suite.tests[2].name,
            'A scenario with examples (3)',
            'Test 3 should have the right name'
          );
      ...
      'in functional tests "remote" should be part of the World'() {
        cucumberInterface.default(
          ...
          'Feature: ...\nScenario: A scenario\nGiven x = 5',
          () => {
            cucumberInterface.Given('x = 5', function() {
              // Note: using `function` is important here!
              assert.isDefined(
                this.remote,
                '"remote" should be part of the World'
              );
      ...
      'failing steps should give an error'() {
        cucumberInterface.default(
          ...
          () => {
            cucumberInterface.Given('x = 5', () => {});
            cucumberInterface.Given('y = 5', () => {
              assert.ok(false, 'This fails');
            });
          }
        );
        return rootSuite.run().then(() => {
