...
class CucumberSuite extends Suite.default {
    ...
    _createEventBroadcaster() {
        ...
        this.eventDataCollector = new cucumber.formatterHelpers.EventDataCollector(this.eventBroadcaster);
        let test, used;
        ...
        this.eventBroadcaster.on('test-case-started', (event) => {
            try {
                let data = this.eventDataCollector.getTestCaseData(event.sourceLocation);
...
class CucumberInterface {
    ...
    registerCucumber(name, featureSource, ...stepDefinitionInitializers) {
        this.executor.addSuite((parent) => {
            this.suite = new CucumberSuite({ name, featureSource, stepDefinitionInitializers });
...
let globalInterface = getInterface(global.default.intern);
